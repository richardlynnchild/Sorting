...
public class Sort_Utils
{
	...
	{
		if(sum_array(array1) == sum_array(array2)){
			return true;
		}
		return false;
	}
	...
	/**
	 * Helper method to sum all elements of an array list of integers
	 * together.
	 * @param array
	 * @return sum - the sum of all elements of the array passed
	 */
	private static int sum_array(ArrayList<Integer> array){
		int sum = 0;
	...
	protected static ArrayList<Integer> generate_random_array( int size, int max )
	{
		...
		ArrayList<Integer> array = new ArrayList<Integer>();
		for(int index = 0; index < size; index++){
			array.add(random.nextInt(max+1));
		}
		return array;
	}
									...
									int timeout_threshold_seconds )
	{
		...
		for (int count = start_count; count <= max_count; count += count_increment)
		{
			...
			// if (sort_routine.get_expected_complexity_class() IS N log N then divide time by N Log N
			{
	...
	private double get_time()
	{
		...
		return (stop-start)/1_000_000_000;
	}
